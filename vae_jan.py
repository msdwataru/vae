...
class VariationalAutoencoder(object):
    ...
    def partial_fit(self, X):
        ...
        opt, cost, lat_cost = self.sess.run((self.optimizer, self.cost, self.latent_loss), 
                                  feed_dict={self.x: X})
        return cost, lat_cost
...
def train(network_architecture, learning_rate=0.001,
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            cost, lat_cost = vae.partial_fit(batch_xs)
            # Compute average loss
            avg_cost += cost / n_samples * batch_size
            avg_lat_cost += lat_cost / n_samples * batch_size

        # Display logs per epoch step
        if epoch % display_step == 0:
            print("Epoch:", '%04d' % (epoch+1), 
                  "cost=", "{:.9f}".format(avg_cost),
                  "latent cost=", "{:.9f}".format(avg_lat_cost))
...
vae_2d = train(network_architecture, training_epochs=1)
