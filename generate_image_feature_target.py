...
flags.DEFINE_string("data_dir", "./image/20180904/", "Directory of training data")
...
def main(_):
    ...
    data, data_num, data_shape, seq_num = read_images(FLAGS.data_dir)
    seq_length = data_num / seq_num
    ...
    outputs, mu, sigma, latent_variable = vae(input_placeholder, seq_length, train=True)
    ...
    with tf.Session() as sess:
        ...
        for i in range(seq_num):
            batch_x = batch_xs[i * seq_length:(i + 1) * seq_length]
            reconstructed_images, mu_log = sess.run([outputs, mu], feed_dict={input_placeholder: batch_x, target_placeholder: batch_x})
            #mu_and_labels = np.c_[mu_log, labels]
            np.savetxt(FLAGS.save_dir + "/feature%.6d.log" % i, mu_log)
